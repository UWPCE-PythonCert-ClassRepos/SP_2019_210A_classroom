...
def test_head():
    hd = Head("head text here")
    hd.append("more head text here")

    file_contents = render_result(hd).strip()

    assert "head text here" in file_contents
    assert "more head text here" in file_contents

    #testing subelement

    page = Html()
    page.append("this is the html")
    page.append(Head("and this is the head"))
    file_contents = render_result(page)
    assert "this is the html" in file_contents
    assert "and this is the head" in file_contents
    # but make sure the embedded element's tags get rendered!
    assert "<head>" in file_contents
    assert "</head>" in file_contents

def test_onelinetag():
    olt = OneLineTag("hello, its me")
    olt.append("calling from the same line")

    file_contents = render_result(olt).strip()

    assert "hello, its me" in file_contents
    assert "calling from the same line" in file_contents

def test_title():
    e = Title("Here is some title text")
