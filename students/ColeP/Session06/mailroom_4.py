...
def gen_stats_print(in_dict):  # prints donor Name, sum of donations, and average donation, formats
    ...
    form_str_table = ['{:22s} ${:11.2f} {:^13}$ {:>12.2f}\n'.format(*row) for row in sorted_donors]
    return ''.join(form_str_table)

    # print(form_str_table)
...
def table_header():
    return "\n{:22s} _ {:11s} _ {:9s} _ {:12s}".format("Donor Name", "Total Given", "Num Gifts", "Average Gift\n")
